...
def run_random_eig_session(
    ...
    """Random selection baseline within the BED framework.

    Generates the same LLM candidates as EIG but randomly selects one
    instead of scoring by EIG. Belief is updated via Bayesian update
    (same as EIG), isolating the value of the EIG scoring layer.

    Note: patient_info, scoring_method, top_k, top_to_show_when_contrastive,
    stagnation_*, concentration_* are unused but accepted for unified interface.
    """
    ...
    for turn in range(max_q):
        ...
        # Generate candidate questions and tests (same as EIG)
        q_texts = agent.generate_questions(current_probs, history, k=question_num)
        t_texts = agent.propose_tests(current_probs, history, k=test_num)
        ...
        chosen_q = random.choice(valid_qs)

        # Determine cost and get likelihoods
        if '<test>' in chosen_q:
            likelihoods = oracle.get_likelihood_matrix_for_test(chosen_q, diagnoses)
            associated_cost = agent.estimate_test_cost(chosen_q)
        else:
            likelihoods = oracle.yes_probabilities(diagnoses, chosen_q)
            associated_cost = question_cost

        is_test = '<test>' in chosen_q
        print(f"  Turn {turn}: {'TEST' if is_test else 'Q'} | RANDOM | cost={associated_cost} | {chosen_q[:60]}")

        # Get answer from oracle
        p_yes = likelihoods[true_diagnosis]
        answer = p_yes >= 0.5

        history.append(QAPair(chosen_q, answer))
        curr_cost += associated_cost

        # Bayesian update (same as EIG)
        current_probs = bayesian_update(current_probs, likelihoods, answer)
