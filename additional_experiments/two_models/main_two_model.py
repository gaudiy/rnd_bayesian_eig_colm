#!/usr/bin/env python3
"""
Worker process for two-model experiments via vLLM.

Oracle  → strong model (27b) via vllm_oracle_base_url
Agent   → weak model   (4b)  via vllm_agent_base_url

get_likelihood_matrix_for_test is now on Oracle, so test likelihoods
always use the strong model — consistent with question likelihoods.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Medical Diagnosis Experiment - Two-Model vLLM Worker")
    ...
    oracle_base_url  = cfg.get('vllm_oracle_base_url', 'http://localhost:8234/v1')
    agent_base_url   = cfg.get('vllm_agent_base_url',  'http://localhost:8235/v1')
    oracle_model     = cfg['oracle_model_name']
    agent_model      = cfg['agent_model_name']

    print("=" * 80)
    print(f"[{ts()}] MEDICAL DIAGNOSIS EXPERIMENT (TWO-MODEL) - WORKER {worker_id}")
    print(f"[{ts()}] Oracle : {oracle_model} @ {oracle_base_url}")
    print(f"[{ts()}] Agent  : {agent_model} @ {agent_base_url}")
    print("=" * 80)
    ...
    # --- Two vLLM clients ---
    oracle_client = OpenAI(base_url=oracle_base_url, api_key="dummy")
    agent_client  = OpenAI(base_url=agent_base_url,  api_key="dummy")

    for name, client, url in [("Oracle", oracle_client, oracle_base_url),
                               ("Agent",  agent_client,  agent_base_url)]:
        try:
            client.models.list()
            print(f"\n{name} vLLM server reachable at {url}")
        except Exception as e:
            print(f"\nWARNING: Could not reach {name} vLLM server: {e}")
            print("Continuing anyway — requests will fail if server is not running.")

    print("\nInitializing medical agent...")
    agent = MedicalAgent(agent_client, agent_model, disease_list)
    ...
    while True:
        ...
        try:
            ...
            oracle = Oracle(oracle_client, oracle_model, true_diagnosis)
